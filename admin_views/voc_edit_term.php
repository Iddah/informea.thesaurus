...
<script src="<?php bloginfo('template_directory'); ?>/scripts/dhtmlxmenu/ext/dhtmlxmenu_ext.js"
        type="text/javascript"></script>
<script src="<?php bloginfo('url'); ?>/wp-content/plugins/thesaurus/admin_views/voc_edit_term.js"
        type="text/javascript"></script>

<link rel="stylesheet" type="text/css"
      href="<?php bloginfo('template_directory'); ?>/scripts/dhtmlxtree/dhtmlxtree.css"/>
<link rel="stylesheet" type="text/css"
...
<div class="wrap">
    ...
    View <a href="<?php bloginfo('url'); ?>/terms">list of terms</a> or <a
        href="<?php bloginfo('url'); ?>/terms/<?php echo $id_term; ?>">term index page</a> in front-end.
    <p>
    ...
    <form id="form_voc_edit_term"
          action="<?php bloginfo('url'); ?>/wp-admin/admin.php?page=thesaurus&act=voc_relationships" method="post">
        ...
        <div class="field">
            ...
            <textarea class="input" rows="5" cols="40" id="description" name="description"
                      value=""><?php echo $term->description;?></textarea>
        </div>

        <div class="field">
            <label for="reference_url">Reference URL</label>
            <input type="text" size="42" id="reference_url" name="reference_url"
                   value="<?php echo $term->reference_url; ?>"/>
        </div>
        ...
        <div class="field">
            ...
            <input type="checkbox" id="top_concept" name="top_concept" value="1" <?php if ($term->top_concept) {
        ...
        <div class="field">
            ...
            <input type="text" size="60" id="geg_tools_url" name="geg_tools_url"
                   value="<?php echo $term->geg_tools_url; ?>"/>
        </div>

        <h3>Term relationships</h3>
        <a href="<?php bloginfo('url'); ?>/wp-admin/admin.php?page=thesaurus&amp;act=voc_manage_terms&amp;expand=<?php echo $term->id; ?>">View
            hierarhical relationships</a>
        <?php if (!$term->top_concept) { ?>
            <div class="field">
                <label for="broader">
                    Broader terms
                    <br/>
                    <em style="color: red;">Use <strong>(Ctrl, Shift) + Click</strong> to select/deselect <br/>multiple
                ...
                <select id="broader" name="broader[]" size="12" multiple="multiple" style="width: 360px; height: 25em">
                    ...
                    foreach ($page_data->get_voc_concept($id_term) as $row) {
                        ...
                        ?>
                        <option title="<?php echo $label; ?>"
                                value="<?php echo $row->id; ?>"<?php echo $checked; ?>><?php echo $label; ?></option>
                    <?php }?>
                </select>
            </div>
        <?php } ?>
        ...
        <div class="field">
            ...
            <input type="text" id="syn_suggest" name="syn_suggest" size="40"/><input id="syn_suggest_add" type="button"
                                                                                     value="Add"/>
        </div>
    ...
    <form action="" method="post"
          onsubmit="return confirm('This will delete PERMANENTLY the term and its relationships! Are you sure?');">
        <?php wp_nonce_field('informea-admin_voc_relationships'); ?>
        <input type="hidden" name="page" value="thesaurus"/>
        <input type="hidden" name="act" value="voc_relationships"/>
